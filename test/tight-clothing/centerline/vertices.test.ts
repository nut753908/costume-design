import {
  createRemainingVerticesMap,
  findNextVertex,
} from "src/tight-clothing/centerline/vertices";
import {
  beforeEach,
  describe,
  expect,
  type MockInstance,
  test,
  vi,
} from "vitest";

describe("createRemainingVerticesMap()", () => {
  test("triangular pyramid example", () => {
    // See the triangular pyramid example in test/body/body-geometry.test.
    const nPolygonIndices = [
      [0, 1, 2],
      [0, 3, 1],
      [1, 3, 2],
      [2, 3, 0],
    ];
    const expected = {
      "0,1": [[2], [3]],
      "1,0": [[2], [3]],
      "1,2": [[0], [3]],
      "2,1": [[0], [3]],
      "2,0": [[1], [3]],
      "0,2": [[1], [3]],
      "0,3": [[1], [2]],
      "3,0": [[1], [2]],
      "3,1": [[0], [2]],
      "1,3": [[0], [2]],
      "3,2": [[1], [0]],
      "2,3": [[1], [0]],
    };
    expect(createRemainingVerticesMap(nPolygonIndices)).toEqual(expected);
  });

  test("cube example", () => {
    // See the cube example in test/body/body-geometry.test.
    const nPolygonIndices = [
      [0, 1, 2, 3],
      [0, 4, 5, 1],
      [1, 5, 6, 2],
      [2, 6, 7, 3],
      [3, 7, 4, 0],
      [7, 6, 5, 4],
    ];
    const expected = {
      // To avoid formatting when saving in vscode, use Array(...) instead of [...].
      // biome-ignore-start lint/style/useArrayLiterals: To avoid formatting.
      "0,1": Array([2, 3], [4, 5]),
      "1,0": Array([2, 3], [4, 5]),
      "1,2": Array([3, 0], [5, 6]),
      "2,1": Array([3, 0], [5, 6]),
      "2,3": Array([0, 1], [6, 7]),
      "3,2": Array([0, 1], [6, 7]),
      "3,0": Array([1, 2], [7, 4]),
      "0,3": Array([1, 2], [7, 4]),
      "0,4": Array([5, 1], [3, 7]),
      "4,0": Array([5, 1], [3, 7]),
      "4,5": Array([1, 0], [7, 6]),
      "5,4": Array([1, 0], [7, 6]),
      "5,1": Array([0, 4], [6, 2]),
      "1,5": Array([0, 4], [6, 2]),
      "5,6": Array([2, 1], [4, 7]),
      "6,5": Array([2, 1], [4, 7]),
      "6,2": Array([1, 5], [7, 3]),
      "2,6": Array([1, 5], [7, 3]),
      "7,3": Array([2, 6], [4, 0]),
      "3,7": Array([2, 6], [4, 0]),
      "6,7": Array([3, 2], [5, 4]),
      "7,6": Array([3, 2], [5, 4]),
      "4,7": Array([0, 3], [6, 5]),
      "7,4": Array([0, 3], [6, 5]),
      // biome-ignore-end lint/style/useArrayLiterals: To avoid formatting.
    };
    expect(createRemainingVerticesMap(nPolygonIndices)).toEqual(expected);
  });

  test("plane example", () => {
    // See a similar plane example in test/body/body-geometry.test.
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [10, 20, 21, 11],
      [11, 21, 22, 12],
    ];
    const expected = {
      // To avoid formatting when saving in vscode, use Array(...) instead of [...].
      // biome-ignore-start lint/style/useArrayLiterals: To avoid formatting.
      "0,10": Array([11, 1]),
      "10,0": Array([11, 1]),
      "10,11": Array([1, 0], [20, 21]),
      "11,10": Array([1, 0], [20, 21]),
      "11,1": Array([0, 10], [12, 2]),
      "1,11": Array([0, 10], [12, 2]),
      "1,0": Array([10, 11]),
      "0,1": Array([10, 11]),
      "11,12": Array([2, 1], [21, 22]),
      "12,11": Array([2, 1], [21, 22]),
      "2,1": Array([11, 12]),
      "1,2": Array([11, 12]),
      "12,2": Array([1, 11]),
      "2,12": Array([1, 11]),
      "10,20": Array([21, 11]),
      "20,10": Array([21, 11]),
      "20,21": Array([11, 10]),
      "21,20": Array([11, 10]),
      "21,11": Array([10, 20], [22, 12]),
      "11,21": Array([10, 20], [22, 12]),
      "21,22": Array([12, 11]),
      "22,21": Array([12, 11]),
      "22,12": Array([11, 21]),
      "12,22": Array([11, 21]),
      // biome-ignore-end lint/style/useArrayLiterals: To avoid formatting.
    };
    expect(createRemainingVerticesMap(nPolygonIndices)).toEqual(expected);
  });
});

describe("findNextVertex()", () => {
  let spy: MockInstance;

  beforeEach(() => {
    spy = vi.spyOn(console, "error");
  });

  test("find it once", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [10, 20, 21, 11],
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBe(12);
    expect(findNextVertex(map, 11, 12)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("find it twice", () => {
    /**
     * flat layout:
     *   20 21 22 23
     *   10 11 12 13
     *    0  1  2  3
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [2, 12, 13, 3],
      [10, 20, 21, 11],
      [11, 21, 22, 12],
      [12, 22, 23, 13],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBe(12);
    expect(findNextVertex(map, 11, 12)).toBe(13);
    expect(findNextVertex(map, 12, 13)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing top left", () => {
    /**
     * flat layout:
     *      21 22
     *   10 11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing top left (interpolate with triangles)", () => {
    /**
     * flat layout:
     *      21 22
     *   10 11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [10, 21, 11], // interpolate with triangles
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing top right", () => {
    /**
     * flat layout:
     *   20 21
     *   10 11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 1, 12, 2],
      [10, 20, 21, 11],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing top right (interpolate with triangles)", () => {
    /**
     * flat layout:
     *   20 21
     *   10 11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [10, 20, 21, 11],
      [11, 21, 12], // interpolate with triangles
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing bottom left", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12
     *       1  2
     */
    const nPolygonIndices = [
      [1, 11, 12, 2],
      [10, 20, 21, 11],
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing bottom left (interpolate with triangles)", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12
     *       1  2
     */
    const nPolygonIndices = [
      [10, 11, 1], // interpolate with triangles
      [1, 11, 12, 2],
      [10, 20, 21, 11],
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing bottom right", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12
     *    0  1
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [10, 20, 21, 11],
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing bottom right (interpolate with triangles)", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12
     *    0  1
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12], // interpolate with triangles
      [10, 20, 21, 11],
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing v1 (interpolate with triangles)", () => {
    /**
     * flat layout:
     *   20 21 22
     *      11 12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 11, 1], // interpolate with triangles
      [1, 11, 12, 2],
      [20, 21, 11], // interpolate with triangles
      [11, 21, 22, 12],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("missing v2 (interpolate with triangles)", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10    12
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 1], // interpolate with triangles
      [1, 12, 2], // interpolate with triangles
      [10, 20, 21], // interpolate with triangles
      [21, 22, 12], // interpolate with triangles
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });

  test("the two vertices are different", () => {
    /**
     * flat layout:
     *   20 21 22
     *   10 11 12(112)
     *    0  1  2
     */
    const nPolygonIndices = [
      [0, 10, 11, 1],
      [1, 11, 12, 2],
      [10, 20, 21, 11],
      [11, 21, 22, 112],
    ];
    const map = createRemainingVerticesMap(nPolygonIndices);
    expect(findNextVertex(map, 10, 11)).toBeNull();
    expect(spy).toHaveBeenCalledTimes(0);
  });
});
